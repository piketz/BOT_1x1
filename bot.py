...
@bot.command(brief='Регистрация нового пользователя', description='Через пробел можно указать дополнительно информацию о себе')
...
async def on_raw_reaction_add(payload): #чекает новые реакции
    ...
    if dbg_info_in_channel == 'yes': channel = await bot.fetch_channel('912553521629495336') #dbg канал
    else: channel = await bot.fetch_channel(payload.channel_id) # канал где к боту обратились
 #  if dbg_info_in_channel == 'yes':print(type(payload.channel_id))
    ...
    if str(check_emoji) == '✅' and not id_match == None and not str(id_match[7]) == '1':# #тут код для реакции о победе или проигрыше ✅ 🚫
...
def end_match_elo(win_user_id,los_user_id,id_mess):
    ...
    if user1[6] == None or user2[6] == None: 
        ...
        return
    print(f'DBG_ 🚩 [INFO] end_match_elo : win_user_id={str(win_user_id)} {type(win_user_id)} los_user_id={str(los_user_id)} {type(los_user_id)} id_mess={str(id_mess)} {type(id_mess)}')
   ...
   # result = [int(user1[6]), int(user2[6])]
    ...
    print(f'DBG_ 🚩 new ELO:  { rtn_name_on_id(user1[0])}={(str(result[0]))} : {rtn_name_on_id(user2[0])}={(str(result[1]))} ')
